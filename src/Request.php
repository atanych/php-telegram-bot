...
class Request
{
    ...
    public static function downloadFile(File $file)
    {
        ...
        //get extension
        $url_image = 'https://api.telegram.org/file/bot' . self::$telegram->getApiKey() . '/' . $path;

        $ch_image = curl_init ($url_image);
        curl_setopt($ch_image, CURLOPT_RETURNTRANSFER, 1);
        curl_exec ($ch_image);
        $content_type = curl_getinfo($ch_image, CURLINFO_CONTENT_TYPE);

        $new_path = $pathInfo['dirname'] . '/' . $pathInfo['filename'] . time() . '_' . rand(0, 99999) . array_search($content_type, self::mime_types());
    ...
    protected static function mime_types()
    {
        return [
            '.3dm' => 'x-world/x-3dmf',
            '.3dmf' => 'x-world/x-3dmf',
